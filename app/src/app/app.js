  ...
  const v = el(`<div>
    ...
    <details class="section aa-advanced" ${(aa.runAnytime === false || aa.maxPerDay < 50 || (Number(aa.concurrency) || 1) > 1) ? 'open' : ''}>
      ...
      <div class="queue-controls section-body" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:16px">
        ...
        ${qc('Parallel applications', `<input class="input" id="aa-conc" type="number" min="1" max="3" value="${Math.max(1, Math.min(3, Number(aa.concurrency) || 3))}" /><div class="form-hint">3 (the default) runs separate-site workers in their own owned Chrome windows for higher throughput; each window's form works unfocused, and a backgrounded window that loads slowly is fronted-on-load or retried. 1 = one at a time (lowest automation footprint). The hourly cap still binds total throughput.</div>`)}
    ...
    try {
      ...
      if (conc > 1 && conc > (Math.max(1, Number(aa.concurrency) || 1))) {
        const ok = await confirmModal(
          `This opens ${conc} apply windows at the same time. Note: only ONE window is in the foreground at a time, and Chrome can THROTTLE a backgrounded/occluded apply window while its /apply/ page is still loading — that one window's form may load slowly. JAT mitigates this: the foreground window is brought to front while its form loads, and any window whose form doesn't hydrate retries on its own. The bigger trade-off is automation footprint: running several apply windows in parallel looks more like a bot to LinkedIn/Indeed (higher risk of a temporary block or account flag). "1" (one at a time) is the lowest-risk setting. Continue?`,
