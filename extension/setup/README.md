...
| --- | --- | --- |
| `JAT-v8-setup.exe` | Windows | One-click setup. Installs the desktop app to `%LOCALAPPDATA%\Programs\JobApplicationTrackerV7`, adds Start Menu / Desktop shortcuts, registers the `jat8://` URL handler so the extension can launch the app, and (optionally) sets `OLLAMA_ORIGINS=chrome-extension://*`. |
| `JAT-v8.pkg` | macOS | Standard Apple installer package. Drops the app under `/Applications`. |
| `JAT-v8.AppImage` | Linux | Single-file portable binary. `chmod +x` then run. No system install required. |
| `JAT-v8.deb` | Debian / Ubuntu | Install with `sudo dpkg -i JAT-v8.deb` (or double-click in your file manager). |
...
2. Click **Run bundled installer**. Chrome will save `JAT-v8-setup.exe` to
   your Downloads folder.
3. Open Downloads, double-click `JAT-v8-setup.exe`.
...
### macOS
1. Click **Run bundled installer** on the install page → it downloads `JAT-v8.pkg`.
...
```bash
cd ~/Downloads
chmod +x JAT-v8.AppImage
./JAT-v8.AppImage
```

**.deb (Ubuntu / Debian):**
```bash
sudo dpkg -i JAT-v8.deb
...
- macOS: `v8/app/build/build-mac.sh`
- Linux: `v8/app/build/build-linux.sh`
