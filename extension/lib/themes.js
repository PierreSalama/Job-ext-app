// 20+ built-in themes. Each theme defines the CSS custom properties
// applied to the entire app via data-theme="<id>" on document.body.
// All themes provide: bg, bg2, panel, border, text, muted, primary, primary2, success, warn, danger.
export const THEMES = [
  // ========== Dark themes ==========
  { id: 'midnight', name: 'Midnight (default)', icon: '🌙', mode: 'dark', vars: { bg: '#0f172a', bg2: '#1e293b', panel: '#1e293b', border: 'rgba(255,255,255,0.08)', text: '#f8fafc', muted: '#94a3b8', primary: '#6366f1', primary2: '#8b5cf6', success: '#10b981', warn: '#f59e0b', danger: '#ef4444' } },
...
// any extension page. Returns a no-op unsubscriber when chrome.storage isn't
// available (e.g. in the Electron renderer).
export function subscribeThemeChanges(cb) {
  ...
  const handler = (changes, area) => {
    ...
    for (const k of ['jat8.settings', 'jat5.settings']) {
      ...
      const oldTheme = c.oldValue?.theme;
